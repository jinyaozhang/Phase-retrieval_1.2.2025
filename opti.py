...
def conjugate_gradient(alpha0, max_iter,  current_phase1_est, amp1, amp2, m, n, z_vec1, wavelength1, n_0, sampling,
    ...
    for k in range(max_iter):
        ...
        costs[k] = cost
        phase_err[k] = np.sum(phase1_es - phase0)/(m*n)
        ...
        if k % 10 == 0 or k == max_iter - 1:
            print(f"Iteration {k}: Cost = {cost.numpy():.2e}")
    return phase1_es.numpy(), amp2_es.numpy(), costs, phase_err


def gradient_descent(alpha, iters, phase1_est, amp1, amp2, m, n, z_vec, wavelength, n0, sampling, phase0):
